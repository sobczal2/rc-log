...
function difficultyName(type: Type, difficulty: DifficultyLevel): string {
  if (type === "Drone") {
    switch (difficulty) {
      case "level1":
        return "Beginner";
...
export function DifficultyBadgeLong({
  ...
  return (
    <Badge
      ...
      className={cn("rounded-sm font-mono font-bold", difficultyColor(difficulty), className)}
    >
      L{n}: {difficultyName(type, difficulty)}
    </Badge>
  );
}
...
export function DifficultyRangeBadgeLong({
  ...
  const label =
    minN === maxN
      ? `L${minN}: ${difficultyName(type, minDifficulty)}`
      : `L${minN}: ${difficultyName(type, minDifficulty)} – L${maxN}: ${difficultyName(type, maxDifficulty)}`;
  return (
    <Badge
      variant="outline"
      className={cn("rounded-sm font-mono font-bold", difficultyColor(maxDifficulty), className)}

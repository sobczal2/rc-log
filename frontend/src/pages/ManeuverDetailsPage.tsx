  ...
  if (isError || !maneuver) {
    return (
      <div className="p-8 text-center flex flex-col items-center justify-center h-full gap-4">
        ...
        <h2 className="text-2xl font-bold">{error instanceof Error ? error.message : "Maneuver not found"}</h2>
    ...
    <div className="flex flex-col h-full w-full overflow-hidden">
      <div className="flex-1 overflow-auto custom-scrollbar">
        <div className="max-w-5xl mx-auto p-4 md:p-8">
          <div className="mb-6">
            <Button variant="ghost" size="sm" className="-ml-3 text-muted-foreground hover:text-foreground" render={<Link to="/maneuvers" />}>
              <ArrowLeft className="mr-2" size={16} /> Back to maneuvers
            </Button>
          </div>
          <div className="flex flex-col md:flex-row gap-8 lg:gap-12">
              ...
              <div className="flex flex-col gap-3 p-5 bg-card/50 shadow-sm border border-border/50 rounded-xl">
                ...
                <div className="flex flex-wrap gap-1.5 mt-2">
                  {maneuver.tags.map(tag => (
                    <Badge key={tag.id} variant="secondary" className="font-normal border-border/50">
                      {tag.name}
                    </Badge>
                  ))}
                </div>
            ...
            <div className="w-full md:w-2/3 flex flex-col pb-16">
              <h1 className="text-3xl md:text-5xl font-bold tracking-tight mb-8 leading-tight">{maneuver.name}</h1>
